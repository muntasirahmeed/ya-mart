...
const DropdownMenu = ({each}) => {
	...
	return (
		<div className="relative z-20 overflow-hidden">
			...
			<div className={`bg-gray-800 ${expanded ? "top-0 relative" : "-top-80 absolute"} overflow-hidden transition-all ease-in-out duration-300  z-0 left-0`}>
				{each?.subMenu?.map((menuItem) => (
					<Link key={menuItem.id} to={menuItem.path} className="pl-11 block py-2 cursor-pointer relative w-full duration-300 hover:bg-slate-700">
						{menuItem.title}
					</Link>
				))}
			</div>
